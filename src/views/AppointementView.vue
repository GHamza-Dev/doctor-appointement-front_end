<template>
  <div class="max-w-xl mt-24 rounded-md m-auto h-96 bg-indigo-300 mx-2">
    <form>
    ...
    <div class="grid grid-cols-4 grid-rows-4 justify-items-stretch items-stretch gap-4 px-3 pt-3">
      <div v-for="sc in $store.state.schedule" :key="sc.scheduleId" class="h-24  border text-center pt-2 relative border-emerald-100 rounded cursor-pointer">
        <p class="text-gray-dark text-lg">{{sc.label}}</p>
